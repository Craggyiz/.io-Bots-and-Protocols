...
function chachaBlock(key32, counter, nonce12) {
    ...
    for (let i = 0; i < 16; i++)
        dv.setUint32(i * 4, (x[i] + base[i]) >>> 0, true);
    return out;
}
...
class Crypto {
    x25519(scalarBytes, uBytes) {
        ...
        const u = (() => {
            const b = Uint8Array.from(uBytes);
            b[31] &= 127;
            let v = 0n;
    ...
    x25519Base(scalarBytes) {
        return this.x25519(scalarBytes, X25519_BASE);
    }
    ...
    chacha20(key32, nonce12, input) {
        ...
        for (let off = 0, blk = 0; off < input.length; off += 64, blk++) {
            ...
            for (let i = 0; i < 64 && off + i < input.length; i++)
    ...
    verifyEd25519(message, signature, publicKey) {
        ...
        try {
            const spki = concatBytes([
                fromHex("302a300506032b6570032100"),
                publicKey,
            ]);
            const key = nCrypto.createPublicKey({
                key: Buffer.from(spki),
                format: "der",
                type: "spki",
            });
            return nCrypto.verify(
                null,
                Buffer.from(message),
                key,
    ...
    for (const f of fields || []) {
        if (typeof f === "string") {
            ...
            if (b.length <= 15) parts.push(new Uint8Array([0xc0 | b.length]), b);
                ...
                parts.push(
                    new Uint8Array([0xfe, b.length & 0xff, (b.length >> 8) & 0xff]),
                    b,
                );
...
class Fingerprint {
    ...
    report(canvasEl) {
        ...
        const lines = [
            ...
            ([label, fn]) =>
                `${label} = ${fn ? Function.prototype.toString.call(fn) : "undefined"}`,
        );
    ...
    collect(overrides = {}) {
        ...
        const pick = (ovKey, live, cap) =>
            overrides[ovKey] ?? (inBrowser ? live() : cap);
        ...
        return {
            ...
            overseer: {
                features: {
                    ...
                    rtc: pick(
                        "rtc",
                        () => (has(g, "RTCPeerConnection") ? "good" : "none"),
                        C.rtc,
                    ),
                    ...
                    credentialless: pick(
                        "credentialless",
                        () => typeof g.credentialless === "boolean",
                        C.credentialless,
                    ),
                    ua,
...
class Connection {
    constructor(socketUrl) {
        ...
        this.serverIdentityKeyHex =
            "98dcbf48d0d78d81d339a2d80bbe85c5d32d7a79eb7223ee9b7c4a54e101d57c";
    ...
    connect() {
        ...
        this.socket = new WebSocket(
            this.socketUrl,
            ["arras.io#v1.4+sls+et0", "arras.io"],
            {
                headers: {
    ...
    buildOpenPacket() {
        ...
        if (!b || !/^[0-9a-fA-F]{16,}$/.test(b))
            throw new Error("URL has no valid 64-bit `b` token");
        const packet = new Uint8Array(12);
        packet.set([0x00, 0x01, 0x00, 0x01], 0);
        let token = BigInt("0x" + b.slice(0, 16));
        for (let i = 0; i < 8; i++) {
            packet[4 + i] = Number(token & 0xffn);
            token >>= 8n;
        }
        return packet;
    }
    ...
    handleMessage(event) {
        ...
        try {
            ...
            if (this.state === "ready") {
                ...
                const char =
                    byte >= 0x20 && byte < 0x7f ? String.fromCharCode(byte) : null;
                ...
                switch (char) {
                    ...
                    case "P":
                        ...
                        break;
                    case 'C':
                        this.sha256PoW(plaintext);
                        break;
                    case 'e':
                        this.runtimeAttestationVM(plaintext);
                        break;
                    default:
                        break;
                }
    ...
    completeHandshake(challenge) {
        ...
        if (this.verifySignature) {
            const ok = this.crypto.verifyEd25519(
                challenge.subarray(0, 32),
                challenge.subarray(32, 96),
                fromHex(this.serverIdentityKeyHex),
            );
            if (!ok) {
        ...
        this.ephemeralScalar[0] &= 248;
        this.ephemeralScalar[31] &= 127;
        this.ephemeralScalar[31] |= 64;
    ...
    // 'e' — JS-VM runtime attestation.
    runtimeAttestationVM(plaintext) {
        try {
            const fields = this.decodeFields(plaintext.subarray(1));
            const token = Buffer.from(fields[0]).toString("latin1");
            const js = Buffer.from(fields[1]).toString("latin1");
            const sandbox = { navigator: { webdriver: false } };
            vm.createContext(sandbox);
            const result = vm.runInContext("(function(){" + js + "})", sandbox, {
                timeout: 2000,
            })();
            this.sendPacket(encodeCommand("e", [token, String(result)]));
        } catch (err) {
            console.error("Failed to answer runtime attestation challenge:", err.message);
            this.close();
        }
    }
    ...
    decodeFields(buf) {
        ...
        while (o < buf.length) {
            ...
            if (m >= 0xc0 && m <= 0xcf) {
                const L = m & 0x0f;
                out.push(buf.subarray(o + 1, o + 1 + L));
                o += 1 + L;
            } else if (m === 0xfe) {
                const L = buf[o + 1] | (buf[o + 2] << 8);
                out.push(buf.subarray(o + 3, o + 3 + L));
                o += 3 + L;
            } else {
                out.push(buf.subarray(o, o + 1));
                o += 1;
            }
        }
        return out;
    ...
    // 'C' — runtime proof-of-work attestation
    sha256PoW(plaintext) {
        try {
            const token = Buffer.from(this.decodeFields(plaintext.subarray(1))[0]);
            const enc = (v) => {
            ...
            let answer = null;
            for (let v = 0; v < 1 << 24; v++) {
                const nonce = enc(v);
                const h = this.crypto.sha256(nonce, token); // SHA256(enc(v) ‖ token)
                if (h[0] === 0 && h[1] === 0) {
                    answer = nonce;
                    break;
                }
            ...
            if (!answer) {
                console.error("PoW challenge: no valid nonce found");
                return;
            }
            this.sendPacket(
                encodeCommand("R", [
                    Buffer.from(token).toString("latin1"),
                    Buffer.from(answer).toString("latin1"),
                ]),
            );
        } catch (err) {
            console.error("Failed to answer PoW challenge:", err.message);
        }
    }
    ...
    buildFingerprintPacket(fingerprint, overrides) {
        const value =
            fingerprint != null
                ? fingerprint
                : this.fingerprint.collect(overrides || {});
    ...
    decrypt(wire) {
        ...
        const expected = this.crypto
            .sha256(ciphertext, this.key, nonce8)
            .subarray(0, 6);
        let diff = 0;
        for (let i = 0; i < 6; i++) diff |= expected[i] ^ receivedTag[i];
        if (diff !== 0) throw new Error("bad inbound tag (key or nonce desync)");
        const plaintext = this.crypto.chacha20(
            this.key,
            this.chachaNonce(nonce8),
            ciphertext,
        );
        this.inboundCounter++;
        return plaintext;
    ...
    sendPacket(data) {
        ...
        const ciphertext = this.crypto.chacha20(
            this.key,
            this.chachaNonce(nonce8),
            plaintext,
        );
...
// You need to swap b & t param for the latest WASM clients.
const connection = new Connection(
    "wss://qrp6ujau11f36bnm-c.uvwx.xyz:8443/5005/?a=3&b=16ca24fda493c5df&t=1783841037",
);
