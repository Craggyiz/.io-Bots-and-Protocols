...
// Pure-JS SHA-256, used only when Node crypto is absent (e.g. bundled browser).
...
// ============================================================================
// Crypto — every cryptographic operation the handshake needs. Instantiate once
// (Socket does this internally) and reuse.
// ============================================================================
class Crypto {
    // X25519 (RFC 7748): clamps the scalar, masks the u high bit.
    ...
    // Our public key = X25519(scalar, base point u=9).
    x25519Base(scalarBytes) { return this.x25519(scalarBytes, X25519_BASE); }

    // ChaCha20 keystream XOR (RFC 8439). nonce12 = 12-byte nonce; block counter 0.
    ...
    // SHA-256 of the concatenation of all byte-array parts.
    sha256(...parts) {
    ...
    // Ed25519 verify of the challenge signature (Node only; skips if unavailable).
