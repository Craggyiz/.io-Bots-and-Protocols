...
(function() {
    ...
    if (typeof WebSocket === 'undefined' || typeof DataView === 'undefined' ||
        ...
        alert('您的瀏覽器不支援所需的功能，請更新您的瀏覽器或使用其他瀏覽器，以獲得最佳遊戲體驗。');
        window.stop();
    }
    ...
    */
    ...
    function wsMessage(data) {
        ...
        switch (packetId) {
            ...
            case 0x63: { // chat message
                ...
                if (flags.server && name !== 'SERVER') name = `[伺服器] ${name}`;
                if (flags.admin) name = `[管理員] ${name}`;
                if (flags.mod) name = `[主持人] ${name}`;
    ...
    function buildGallery() {
        const sortedSkins = Array.from(knownSkins.keys()).sort((x,y) => x - y);
        let c = '';
        for (const skin of sortedSkins) {
            c += `<li class="skin" onclick="changeSkin('${skin}')">`;
            c += `<div class="skin-image"><img class="circular" src="./skins/${skin}.png"></div>`;
            c += `<h4 class="skinName">${skin}</h4>`;
            c += '</li>';
        }
        byId('gallery-body').innerHTML = `<ul id="skinsUL" class="skins-ul">${c}</ul>`;
    }
    ...
    function drawStats() {
        ...
        const rows = [
            ...
            `${stats.info.playersTotal} / ${stats.info.playersLimit} 線上人數`,
            `${stats.info.playersAlive} 人遊戲中`,
            `${stats.info.playersSpect} 人觀戰中`,
            `伺服器負載 ${(stats.info.update * 2.5).toFixed(1)}% @ 已開服 ${uptime}`,
        ];
        let width = 0;
    ...
    function drawLeaderboard() {
        ...
        ctx.fillText('排行榜', 100 - ctx.measureText('排行榜').width / 2, 40);
    ...
    function drawGame() {
        ...
        if (!isNaN(stats.score)) {
            ...
            mainCtx.fillText(`分數： ${stats.score}`, 2, height);
            height += 20;
        }
        mainCtx.font = '20px Ubuntu';
        const gameStatsText = `${~~stats.fps} FPS` + (isNaN(stats.latency) ? '' : ` Ping ${stats.latency}ms`);
    ...
    function getServersStats() {
        var servers = ["//play.agario.xingkong.tw:8080/", "//play.agario.xingkong.tw:8081/", "//play.agario.xingkong.tw:8082/"];
        var servers_names = ["自由模式", "團隊模式", "實驗模式"];
        servers.forEach(getServersData);
        
        function getServersData(item, index) {
             const server = `${item}`;
                fetch(server, {
                method: 'GET',
                headers: {
            ...
            .then(data => {
                console.log(index);
                document.getElementById('option_' + index).innerHTML = servers_names[index] + " ( 線上 " + data.current_players + " / " + data.max_players + " 人 )";
